...
class Recognize(Recognize_Abstract):
    ...
    def __combine(image, underPlexy):
        x0, x1 = int(190 * 0.5), int(1080 * 0.5)
        y0, y1 = int(910 * 0.5), int(1460 * 0.5)
    ...
    def __resolveLocationFromConturs(self, contours):
        ...
        if not self.oldLocation:
            # Select the largest plausible movement candidate instead of
            # staying permanently at the default edge position.
