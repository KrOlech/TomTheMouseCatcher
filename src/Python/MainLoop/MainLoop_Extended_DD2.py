...
class MainLoop(BaseMainLoop):
    """Extended DD2: L2 reward 100%, L1 reward 25%, delayed opening of D4 in zone D."""
    ...
    def mainLoop(self):
        ...
        while not self.finishFlag.is_set():
            ...
            if zone == "A":
                ...
                self._closeDoors(["D1", "D2", "D4"])
                self._openDoors(["D3", "D5", "D6"])
    ...
    def activate_d4_with_delay(self):
        ...
        def delayed_open():
            try:
                ...
                if (
                    ...
                    self.OpenDoor("D4")
                    self.loger(f"D4 opened after {delay} s (counter={initial_counter})")
                else:
                    self.loger("D4 delayed opening cancelled: context changed")
            finally:
                self._delayed_door_pending = False
